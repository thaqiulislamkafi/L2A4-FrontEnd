...
export default function UpdateUserDialog({ user, open, onOpenChange }: UpdateUserDialogProps) {
  ...
  const [formData, setFormData] = React.useState<UpdateUserPayload>({
    name: "",
    image: "",
    contact: "",
    age: null,
      ...
      <Dialog open={open} onOpenChange={handleClose}>
        <DialogContent className="max-h-[90vh] min-w-lg overflow-y-auto rounded-2xl border-orange-200/80 bg-white p-0 shadow-2xl shadow-orange-950/10 dark:border-orange-900/50 dark:bg-orange-950/95">
          ...
          <DialogHeader className="border-b border-orange-100 px-6 py-5 dark:border-orange-900/40">
            <DialogTitle className=" font-bold text-orange-950 dark:text-orange-50">Update User Information</DialogTitle>
            <DialogDescription className="text-sm text-orange-700/60 dark:text-orange-300/60">Update the profile information of {user?.name ?? "this user"}.</DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={handleSubmit}>
            <div className="space-y-5 px-6 py-5">
              <Card className="border-orange-200/70 bg-orange-50/40 p-4 shadow-none dark:border-orange-900/40 dark:bg-orange-950/20">
                <div className="flex items-center justify-between gap-4">
                  <div className="flex min-w-0 items-center gap-3">
                    {formData.image ? (
                      <Image height={56} width={56} src={formData.image} alt={formData.name || "User"} className="size-14 shrink-0 rounded-xl object-cover ring-2 ring-orange-200 dark:ring-orange-900/60" />
                    ) : (
                    ...
                    <div className="min-w-0">
                      <p className="truncate text-sm font-bold text-orange-950 dark:text-orange-50">{user?.name ?? "User"}</p>
                      <p className="truncate text-xs font-medium text-orange-700/60 dark:text-orange-300/60">{user?.email ?? ""}</p>
                    </div>
                  </div>
              ...
              <div className="space-y-2">
                <Label htmlFor="user-name" className="font-semibold text-orange-900 dark:text-orange-100">Name</Label>
                <Input id="user-name" value={formData.name} onChange={(event) => handleChange("name", event.target.value)} placeholder="Enter user name" disabled={updateMutation.isPending} className="border-orange-200 focus-visible:border-orange-500 focus-visible:ring-orange-500/20 dark:border-orange-800 dark:bg-orange-950/30" />
              </div>

              <div className="space-y-2">
                <Label htmlFor="user-image" className="font-semibold text-orange-900 dark:text-orange-100">Profile Image URL</Label>
                <Input id="user-image" value={formData.image} onChange={(event) => handleChange("image", event.target.value)} placeholder="https://example.com/image.jpg" disabled={updateMutation.isPending} className="border-orange-200 focus-visible:border-orange-500 focus-visible:ring-orange-500/20 dark:border-orange-800 dark:bg-orange-950/30" />
              </div>

              <div className="grid gap-5 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="user-contact" className="font-semibold text-orange-900 dark:text-orange-100">Contact</Label>
                  <Input id="user-contact" value={formData.contact} onChange={(event) => handleChange("contact", event.target.value)} placeholder="+8801XXXXXXXXX" disabled={updateMutation.isPending} className="border-orange-200 focus-visible:border-orange-500 focus-visible:ring-orange-500/20 dark:border-orange-800 dark:bg-orange-950/30" />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="user-age" className="font-semibold text-orange-900 dark:text-orange-100">Age</Label>
                  <Input id="user-age" type="number" min="1" value={formData.age ?? ""} onChange={(event) => handleChange("age", event.target.value)} placeholder="Enter age" disabled={updateMutation.isPending} className="border-orange-200 focus-visible:border-orange-500 focus-visible:ring-orange-500/20 dark:border-orange-800 dark:bg-orange-950/30" />
                </div>
              </div>

              <div className="space-y-2">
                <Label htmlFor="user-address" className="font-semibold text-orange-900 dark:text-orange-100">Address</Label>
                <Input id="user-address" value={formData.address} onChange={(event) => handleChange("address", event.target.value)} placeholder="Enter user address" disabled={updateMutation.isPending} className="border-orange-200 focus-visible:border-orange-500 focus-visible:ring-orange-500/20 dark:border-orange-800 dark:bg-orange-950/30" />
              </div>
            ...
            <DialogFooter className="border-t border-orange-100 px-6 py-4 dark:border-orange-900/40">
              <Button type="button" variant="outline" onClick={() => handleClose(false)} disabled={updateMutation.isPending} className="rounded-lg border-orange-200 text-orange-700 hover:bg-orange-50 hover:text-orange-700 dark:border-orange-800 dark:text-orange-300 dark:hover:bg-orange-950/40">Cancel</Button>

              <Button type="submit" disabled={updateMutation.isPending || !formData.name.trim()} className="rounded-lg bg-orange-500 font-semibold text-white shadow-sm shadow-orange-500/20 hover:bg-orange-600">
      ...
      <Dialog open={imageDialogOpen} onOpenChange={handleImageDialogClose}>
        <DialogContent className="max-w-md rounded-2xl border-orange-200/80 bg-white shadow-2xl shadow-orange-950/10 dark:border-orange-900/50 dark:bg-orange-950/95">
          <DialogHeader>
            ...
            <DialogDescription className="text-orange-700/60 dark:text-orange-300/60">Select a new profile image for {user?.name ?? "this user"}.</DialogDescription>
          </DialogHeader>
            ...
            <div className="flex justify-center">
              {selectedFile ? (
                <div className="relative size-32 overflow-hidden rounded-2xl ring-2 ring-orange-200 dark:ring-orange-900/60">
                  <Image src={URL.createObjectURL(selectedFile)} alt="Selected profile image" fill className="object-cover" />
                </div>
              ) : formData.image ? (
                <div className="relative size-32 overflow-hidden rounded-2xl ring-2 ring-orange-200 dark:ring-orange-900/60">
                  <Image src={formData.image} alt={user?.name ?? "Current profile image"} fill className="object-cover" />
                </div>
              ) : (
