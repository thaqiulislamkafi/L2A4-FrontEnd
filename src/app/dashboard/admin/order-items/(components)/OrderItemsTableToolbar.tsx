...
interface OrderItemsTableToolbarProps {
  search: string;
  onSearchChange: (value: string) => void;
  onReset: () => void;
}

export default function OrderItemsTableToolbar({
  search,
  onSearchChange,
  onReset,
}: OrderItemsTableToolbarProps) {
    ...
    <div className="flex flex-col gap-3 border-b border-orange-100 bg-orange-50/40 p-4 dark:border-orange-900/40 dark:bg-orange-950/10 sm:flex-row sm:items-center sm:justify-between">
      <div className="min-w-0">
        ...
        <p className="mt-0.5 text-xs font-medium text-orange-700/60 dark:text-orange-300/60">
          Manage and view items from customer orders.
        </p>
      </div>
      <div className="flex w-full items-center gap-2 sm:w-auto">
        <div className="relative min-w-0 flex-1 sm:w-72">
          ...
          {hasSearch && (
            <button
              ...
              className="absolute right-2 top-1/2 flex size-6 -translate-y-1/2 items-center justify-center rounded-md text-orange-500 transition-colors hover:bg-orange-100 hover:text-orange-700 dark:text-orange-400 dark:hover:bg-orange-950/60 dark:hover:text-orange-300"
            >
        ...
        {hasSearch && (
          <Button
            ...
            variant="outline"
            onClick={onReset}
            className="h-9 shrink-0 rounded-lg border-orange-200 bg-white px-3 font-medium text-orange-700 hover:bg-orange-50 hover:text-orange-700 dark:border-orange-800 dark:bg-orange-950/20 dark:text-orange-300 dark:hover:bg-orange-950/40"
          >
            Reset
          </Button>
        )}
