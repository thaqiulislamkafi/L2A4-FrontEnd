...
interface ReviewDeleteDialogProps {
  ...
  onConfirm: (review: GlobalReview) => void;
}

export default function ReviewDeleteDialog({ review, open, isDeleting, onOpenChange, onConfirm }: ReviewDeleteDialogProps) {
  if (!review) {
    return null;
  }

  const reviewerName = review.user?.name ?? "this review";
    ...
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent className="max-w-md rounded-2xl border-orange-200 bg-white dark:border-orange-900/50 dark:bg-orange-950">
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction onClick={() => onConfirm(review)} disabled={isDeleting} className="rounded-lg bg-red-600 text-white hover:bg-red-700 focus:ring-red-500 dark:bg-red-600 dark:hover:bg-red-700">
            {isDeleting ? (
              <>
