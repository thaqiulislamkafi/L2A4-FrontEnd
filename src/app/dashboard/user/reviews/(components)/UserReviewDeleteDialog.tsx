...
interface UserReviewDeleteDialogProps {
  ...
  onConfirm: (review: UserReview) => void;
}

export default function UserReviewDeleteDialog({
  review,
  open,
  isDeleting,
  onOpenChange,
  onConfirm,
}: UserReviewDeleteDialogProps) {
  if (!review) {
    return null;
  }
      ...
      <AlertDialogContent className="max-w-md rounded-2xl border-orange-200 bg-white dark:border-orange-900/50 dark:bg-orange-950">
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction
            ...
            disabled={isDeleting}
            className="rounded-lg bg-red-600 text-white hover:bg-red-700 focus:ring-red-500 dark:bg-red-600 dark:hover:bg-red-700"
          >
            {isDeleting ? (
              <>
