...
import { updateUserReview, UpdateUserReviewPayload } from "@/lib/api/user-reviews";
import { UserReview } from "@/types/user-review.type";

interface UpdateUserReviewDialogProps {
  review: UserReview | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export default function UpdateUserReviewDialog({ review, open, onOpenChange }: UpdateUserReviewDialogProps) {
  const queryClient = useQueryClient();

  const updateMutation = useMutation({
    mutationFn: (payload: UpdateUserReviewPayload) => {
      if (!review) {
        throw new Error("Review not found");
      }

      return updateUserReview(review.id, payload);
    },
    onSuccess: async () => {
      onOpenChange(false);
      ...
      <DialogContent className="max-h-[90vh] w-[min(92vw,560px)] overflow-y-auto rounded-2xl border-orange-200/80 bg-white p-0 shadow-2xl shadow-orange-950/10 dark:border-orange-900/50 dark:bg-orange-950/95">
        <DialogHeader className="border-b border-orange-100 px-6 py-5 dark:border-orange-900/40">
          ...
          <DialogDescription className="text-sm text-orange-700/60 dark:text-orange-300/60">
            Update your review for meal {review.meal_id.slice(0, 8)}...{review.meal_id.slice(-4)}.
          </DialogDescription>
        </DialogHeader>
            ...
            <form.Field name="comment">
              {(field) => (
                <div className="space-y-2">
                  <Label htmlFor="user-review-comment" className="font-semibold text-orange-900 dark:text-orange-100">
                    Comment
                  </Label>
