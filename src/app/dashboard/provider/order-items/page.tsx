...
import { getProviderOrderItems } from "@/lib/api/order";
import { useAuthStore } from "@/store/auth.store";

const DEFAULT_PAGE = 1;
const DEFAULT_LIMIT = 3;

export default function ProviderOrderItemsPage() {
  const providerId = useAuthStore((state) => state.user?.id);
  const [page, setPage] = React.useState(DEFAULT_PAGE);
  const [search, setSearch] = React.useState("");

  const orderItemsQuery = useQuery({
    queryKey: ["provider-order-items", providerId, page, DEFAULT_LIMIT, search],
    queryFn: () => {
      if (!providerId) {
        throw new Error("Provider account not found. Please sign in again.");
      }

      return getProviderOrderItems(providerId, { page, limit: DEFAULT_LIMIT, search });
    },
    enabled: !!providerId,
    ...
    <div className="space-y-5 p-4 md:p-6 max-w-234">
      <div>
        ...
        <p className="mt-1 text-sm font-medium text-orange-700/60 dark:text-orange-300/60">
          View all items included in your customer orders.
        </p>
      </div>
