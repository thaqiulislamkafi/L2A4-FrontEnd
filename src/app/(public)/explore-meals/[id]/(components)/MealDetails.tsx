/* eslint-disable @typescript-eslint/no-explicit-any */
"use client";

import Image from "next/image";
import { useState } from "react";
import { useMutation } from "@tanstack/react-query";
import { motion } from "framer-motion";
import { ArrowRight, CheckCircle2, Clock3, Loader2, Minus, Package, Plus, ShoppingBag, Utensils } from "lucide-react";

import { MealDetailsType } from "@/types/meal.type";
import { useAuthStore } from "@/store/auth.store";

import { Card, CardContent } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Label } from "@/components/ui/label";
import { Button } from "@/components/ui/button";
import { toast } from "@/components/ui/toast";
import { addCartItem } from "@/lib/api/cartItem";

interface MealDetailsProps {
  meal: MealDetailsType;
}

interface AddCartItemPayload {
  user_id: string;
  meal_id: string;
  quantity: number;
  price: number;
}

const MealDetails = ({ meal }: MealDetailsProps) => {
  const [quantity, setQuantity] = useState(1);

  const user = useAuthStore((state) => state.user);

  const addCartMutation = useMutation({
    mutationFn: (payload: AddCartItemPayload) => addCartItem(payload),
    onSuccess: (response) => {
      toast.add({
        title: "Added to cart!",
        description: `${quantity} ${quantity === 1 ? "piece has" : "pieces have"} been added to your cart.`,
        type: "success",
      });
    },
    onError: (error: any) => {
      console.error("Add to cart failed:", error);

      toast.add({
        title: "Unable to add to cart",
        description: error?.response?.data?.message || "Something went wrong. Please try again.",
        type: "error",
      });
    },
  });

  const handleAddToCart = () => {
    if (!user) {
      toast.add({
        title: "Please sign in",
        description: "You need to sign in before adding meals to your cart.",
        type: "error",
      });

      return;
    }

    if (meal.availabilty_status !== "AVAILABLE") {
      toast.add({
        title: "Meal unavailable",
        description: "This meal is currently unavailable.",
        type: "error",
      });

      return;
    }

    if (meal.availablePieces <= 0) {
      toast.add({
        title: "Out of stock",
        description: "Sorry, this meal is currently out of stock.",
        type: "error",
      });

      return;
    }

    const payload: AddCartItemPayload = {
      user_id: user.id,
      meal_id: meal.id,
      quantity,
      price: Number(meal.pricePerPiece),
    };

    addCartMutation.mutate(payload);
  };

  return (
    <section className="pt-6">
      <Card className="overflow-hidden border-orange-100 bg-background/90 p-0 shadow-xl shadow-orange-100/30 dark:border-orange-950/40 dark:shadow-orange-950/20">
        <div className="grid lg:grid-cols-2">
          {/* ================= IMAGE ================= */}

          <motion.div
            initial={{ opacity: 0, scale: 0.97 }}
            animate={{ opacity: 1, scale: 1 }}
            transition={{ duration: 0.6 }}
            className="relative min-h-100 lg:min-h-130"
          >
            <Image
              src={meal.image}
              alt={meal.name}
              fill
              priority
              sizes="(max-width: 1024px) 100vw, 50vw"
              className="object-cover"
            />

            <div className="absolute inset-0 bg-linear-to-t from-black/60 via-transparent to-transparent" />

            {/* Availability */}

            <div className="absolute right-5 top-5">
              <Badge
                className={meal.availabilty_status === "AVAILABLE" ? "border border-white/20 bg-orange-600 text-white shadow-lg hover:bg-orange-600" : "bg-slate-700 text-white"}
              >
                <CheckCircle2 className="h-3.5 w-3.5" />

                {meal.availabilty_status === "AVAILABLE" ? "Available" : meal.availabilty_status}
              </Badge>
            </div>

            {/* Price */}

            <div className="absolute bottom-6 left-6">
              <div className="rounded-2xl border border-white/20 bg-black/35 px-5 py-3 text-white backdrop-blur-md">
                <Label className="text-xs text-white/70">
                  Price per piece
                </Label>

                <p className="text-3xl font-bold">
                  ৳{meal.pricePerPiece}
                </p>
              </div>
            </div>
          </motion.div>

          {/* ================= CONTENT ================= */}

          <CardContent className="flex flex-col justify-center p-7 sm:p-10">
            <motion.div
              initial={{ opacity: 0, x: 30 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.6 }}
            >
              {/* Badge */}

              <Badge
                variant="outline"
                className="mb-5 border-orange-200 bg-orange-50 text-orange-700 dark:border-orange-900 dark:bg-orange-950/30 dark:text-orange-400"
              >
                <Utensils className="h-3.5 w-3.5" />

                {meal.cuisine_rel.cuisine_type_name}
              </Badge>

              {/* Name */}

              <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white md:text-4xl">
                {meal.name}
              </h1>

              {/* Description */}

              <p className="mt-5 leading-8 text-muted-foreground">
                {meal.description}
              </p>

              {/* Tags */}

              <div className="mt-6 flex flex-wrap gap-2">
                <Badge
                  variant="secondary"
                  className="border border-orange-100 bg-orange-50 text-orange-700 dark:border-orange-900/50 dark:bg-orange-950/30 dark:text-orange-400"
                >
                  {meal.cuisine_rel.cuisine_type_name}
                </Badge>

                <Badge
                  variant="outline"
                  className="border-orange-200 text-orange-700 dark:border-orange-900 dark:text-orange-400"
                >
                  {meal.category_rel.category_name}
                </Badge>

                <Badge
                  variant="outline"
                  className="border-slate-200 text-slate-600 dark:border-slate-700 dark:text-slate-400"
                >
                  {meal.dietry_rel.dietry_type_name}
                </Badge>
              </div>

              {/* Inventory */}

              <div className="mt-8 grid grid-cols-2 gap-4">
                <div className="rounded-2xl border border-orange-100 bg-orange-50/60 p-4 dark:border-orange-950/40 dark:bg-orange-950/10">
                  <Package className="h-5 w-5 text-orange-500" />

                  <Label className="mt-3 block text-xs text-muted-foreground">
                    Available
                  </Label>

                  <p className="mt-1 text-lg font-bold text-foreground">
                    {meal.availablePieces} pieces
                  </p>
                </div>

                <div className="rounded-2xl border border-orange-100 bg-orange-50/60 p-4 dark:border-orange-950/40 dark:bg-orange-950/10">
                  <Clock3 className="h-5 w-5 text-orange-500" />

                  <Label className="mt-3 block text-xs text-muted-foreground">
                    Total Pieces
                  </Label>

                  <p className="mt-1 text-lg font-bold text-foreground">
                    {meal.totalPieces}
                  </p>
                </div>
              </div>

              {/* ================= CART ACTION ================= */}

              <div className="mt-8 flex flex-col gap-4 sm:flex-row sm:items-center">
                {/* Quantity Selector */}

                <div className="flex h-12 w-fit items-center rounded-xl border border-orange-200 bg-orange-50/60 p-1.5 dark:border-orange-900/50 dark:bg-orange-950/20">
                  <Button
                    type="button"
                    variant="ghost"
                    size="icon"
                    disabled={quantity <= 1 || addCartMutation.isPending}
                    onClick={() => setQuantity((prev) => Math.max(1, prev - 1))}
                    className="size-9 rounded-lg text-orange-600 transition-all hover:bg-orange-100 hover:text-orange-700 disabled:opacity-40 dark:text-orange-400 dark:hover:bg-orange-950/40"
                    aria-label="Decrease quantity"
                  >
                    <Minus className="size-4" />
                  </Button>

                  <div className="flex min-w-12 items-center justify-center">
                    <span className="text-base font-bold text-orange-700 dark:text-orange-400">
                      {quantity}
                    </span>
                  </div>

                  <Button
                    type="button"
                    variant="ghost"
                    size="icon"
                    disabled={quantity >= meal.availablePieces || addCartMutation.isPending}
                    onClick={() => setQuantity((prev) => Math.min(meal.availablePieces, prev + 1))}
                    className="size-9 rounded-lg text-orange-600 transition-all hover:bg-orange-100 hover:text-orange-700 disabled:opacity-40 dark:text-orange-400 dark:hover:bg-orange-950/40"
                    aria-label="Increase quantity"
                  >
                    <Plus className="size-4" />
                  </Button>
                </div>

                {/* Add To Cart */}

                <Button
                  type="button"
                  disabled={addCartMutation.isPending || meal.availabilty_status !== "AVAILABLE" || meal.availablePieces <= 0}
                  onClick={handleAddToCart}
                  className="group h-12 flex-1 rounded-xl bg-orange-600 font-semibold text-white shadow-lg shadow-orange-600/20 transition-all hover:-translate-y-0.5 hover:bg-orange-700 hover:shadow-xl hover:shadow-orange-600/30 disabled:pointer-events-none disabled:opacity-60"
                >
                  {addCartMutation.isPending ? (
                    <>
                      <Loader2 className="size-4 animate-spin" />
                      Adding to Cart...
                    </>
                  ) : (
                    <>
                      <ShoppingBag className="size-4" />

                      Add to Cart

                      <ArrowRight className="ml-auto size-4 transition-transform group-hover:translate-x-1" />
                    </>
                  )}
                </Button>
              </div>

              {/* Selected Quantity Info */}

              <div className="mt-3 flex items-center justify-between text-xs text-muted-foreground">
                <span>
                  {quantity} {quantity === 1 ? "piece" : "pieces"} selected
                </span>

                <span>
                  {meal.availablePieces} available
                </span>
              </div>

              {/* Total Price */}

              <motion.div
                key={quantity}
                initial={{ opacity: 0, y: 4 }}
                animate={{ opacity: 1, y: 0 }}
                className="mt-4 flex items-center justify-between rounded-xl border border-orange-100 bg-orange-50/50 px-4 py-3 dark:border-orange-950/40 dark:bg-orange-950/10"
              >
                <Label className="text-sm font-medium text-muted-foreground">
                  Cart total
                </Label>

                <p className="text-lg font-bold text-orange-600 dark:text-orange-400">
                  ৳{Number(meal.pricePerPiece) * quantity}
                </p>
              </motion.div>
            </motion.div>
          </CardContent>
        </div>
      </Card>
    </section>
  );
};

export default MealDetails;